...
//added by pengcheng
//add network coding coef field
void
Interest::SetCoef(uint64_t s)
{
  m_coef = s;
}
void
Interest::SetPathNum(uint32_t s)
{
  m_pathNum = s;
}
...
{
  size_t size = 2 + (1 + 4 + 2 + 1 + (m_name->GetSerializedSize ()) + (2 + 0) + (2 + 0))       +8+4;//modified by zfx +8, coef+pathNum
