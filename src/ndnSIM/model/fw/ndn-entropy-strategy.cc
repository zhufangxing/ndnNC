                                ...
                                Ptr<pit::Entry> pitEntry)
{
  ...
  if( m_faces_withEntroy.size() > 1 )
  {
       ...
       metricFacewithEntroy++)
    {
	   ...
	   * ----------------------------------------------------------------------------
	   */
	  double v_i1 = abs(m_faces_withEntroy.size()*(metricFacewithEntroy->GetStatusFormal()) - status_sum);
	  v1_sum += v_i1;
	  
	  double v_i2 = abs(m_faces_withEntroy.size()*(metricFacewithEntroy->GetSRttFormal()) - m_sRtt_sum);
	  v2_sum += v_i2;
	  
	  m_faces_withEntroy.modify (metricFacewithEntroy,
                      ll::bind (&FaceMetricWithEntroy::SetStatusFormal, ll::_1, v_i1));
					  
      m_faces_withEntroy.modify (metricFacewithEntroy,
                      ll::bind (&FaceMetricWithEntroy::SetSRttFormal, ll::_1, v_i2));
	...
	// status_weight
	//double w_1 = v1_sum / (v1_sum + v2_sum);
	// m_sRtt_weight
	//double w_2 = v2_sum / (v1_sum + v2_sum);
    
    //calculate entropy value for each col
    double k = -1.0 / (log(m_faces_withEntroy.size()));
    NS_LOG_INFO("k="<<k); 
    NS_LOG_INFO("status_entropy_value before multiple k="<<status_entropy_value<<",m_sRtt_entropy_value before multiple k="<<m_sRtt_entropy_value
      );
    status_entropy_value = k * status_entropy_value;
    m_sRtt_entropy_value = k * m_sRtt_entropy_value;
    NS_LOG_INFO("status_entropy_value="<<status_entropy_value<<",m_sRtt_entropy_value="<<m_sRtt_entropy_value
      ); 
    ...
    NS_LOG_INFO("status_entropy_redundancy_rate="<<status_entropy_redundancy_rate<<",m_sRtt_entropy_redundancy_rate="<<m_sRtt_entropy_redundancy_rate
      ); 
  ...
  BOOST_FOREACH (const FaceMetricWithEntroy &metricFacewithEntroy, m_faces_withEntroy.get<i_score> ())
  {
    ...
    NS_LOG_INFO("metricFacewithEntroy.m_status="<<metricFacewithEntroy.GetStatus ()<<",metricFacewithEntroy.m_sRtt="<<metricFacewithEntroy.GetSRtt ()<<
  ...
  */
  ...
  if(fabs( total_score - 0.0) < EPSILON)
  {
    ...
    BOOST_FOREACH (const FaceMetricWithEntroy &metricFacewithEntroy, m_faces_withEntroy.get<i_score> ())
    {
      ...
      if (metricFacewithEntroy.GetStatus () == fib::FaceMetric::NDN_FIB_RED)
	  continue;
      if (!TrySendOutInterest (inFace, metricFacewithEntroy.GetFace (), interest, origPacket, pitEntry))
	  {
	    continue;
	  }
