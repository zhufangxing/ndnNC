...
//added by zfx
Entry::in_iterator
Entry::AddIncoming (Ptr<Face> face, uint64_t coef)
{
  std::pair<in_iterator,bool> ret =
    m_incoming.insert (IncomingFace (face, coef));
...
Entry::coef_iterator
Entry::AddCoef (uint64_t coef)
{
  std::pair<coef_iterator,bool> ret =
    m_coef.insert (coef);
...
//added by zfx
void
Entry::RemoveCoef (uint64_t coef)
{
  m_coef.erase (coef);
}
